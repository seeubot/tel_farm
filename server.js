...
// ==================== GOOGLE AUTH — CALLBACK (Redirects to Custom Scheme) ====================
app.get('/api/auth/google-mobile/callback', async (req, res) => {
  try {
    ...
    // Create the deep link URL with all data
    const deepLink = `agriagent://auth?token=${tokens.access_token}&userId=${user._id}&email=${user.email}&role=${user.role}`;
    
    console.log('[Auth] Redirecting to:', deepLink);
    
    // IMPORTANT: Redirect to the custom scheme instead of showing HTML
    res.redirect(deepLink);
    
  } catch (error) {
    console.error('Callback error:', error);
    // On error, also redirect to the app with error parameter
    const errorDeepLink = `agriagent://auth?error=${encodeURIComponent(error.message)}`;
